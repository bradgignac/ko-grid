function BaseColumn(key, options) {
  ...
  bodyClassOption = options.bodyClass || options.class || '';
  ...
  sortFunction = function (item1, item2) {
    ...
    value1 = ko.utils.unwrapObservable(item1[key]);
    value2 = ko.utils.unwrapObservable(item2[key]);
...
BaseColumn.create = function (key, options) {
  'use strict';

  return new BaseColumn(key, options);
};
